...
- `backend/api/src/infrastructure/store/repositories.ts` defines the persistence contracts.
...
Migration files live in `backend/api/src/db/migrations`. `npm run migrate -w @gym-platform/api` applies unapplied files through the `pg` driver and records them in `schema_migrations`.
