...
The member dashboard profile state in `apps/dashboard/src/members` consumes these records to build:
...
The membership-plan dashboard state in `apps/dashboard/src/membershipPlans` works against the same plan records before and during assignment, and now covers:
...
The Leads & CRM dashboard state in `apps/dashboard/src/leads` uses the same member model ahead of plan assignment. Lead conversion moves a record from `lead` into `trial` or `active`, which then allows the existing member dashboard and membership-assignment flows to take over for plan history, eligibility, and profile summaries.

The Stripe Payments dashboard state in `apps/dashboard/src/payments` uses the same member and membership context to drive front-desk payment collection, history review, and refund workflows. That keeps payment follow-up tied to the same active-member records and standing checks that determine whether a member can continue through the broader membership-assignment and profile flows.

The check-in dashboard state in `apps/dashboard/src/checkIns` uses the same member and membership context to drive front-desk eligibility review, denied-state messaging, and history visibility. That keeps check-in decisions tied to the same active or trialing assignment rules that govern broader membership standing.
