...
describe("staff invite dashboard flow", () => {
    ...
    it("builds custom role create and edit screens", () => {
        ...
        expect(createScreen.permissionGroups.find((group) => group.key === "members")?.summaryLabel).toBe("1 of 4 selected");
        ...
        expect(lockedSystemRole.permissionGroups.every((group) => group.toggles.every((toggle) => toggle.disabled))).toBe(true);
        ...
        expect(invalidCreateScreen.permissionGroups.find((group) => group.key === "gym")?.summaryLabel).toBe("0 of 2 selected");
        expect(submission).toEqual({
            name: "Operations Lead",
            permissions: [Permission.GymRead, Permission.MemberRead]
        });
    ...
    it("builds restricted dashboard views for trainer and front desk roles", () => {
        ...
        expect(frontDeskView.navItems.find((item) => item.href === "/access-control")?.requiredPermissions).toEqual([Permission.AccessRead]);
