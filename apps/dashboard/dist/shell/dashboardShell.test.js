...
import { buildAccountMenu, buildDashboardShellLayout, buildDashboardHomePage, buildDashboardConfirmationModal, buildDashboardCsvUpload, buildDashboardDataTable, buildDashboardDateRangePicker, buildDashboardDetailDrawer, buildDashboardFilterDrawer, buildDashboardImageUpload, buildDashboardToastNotificationCenter, buildGlobalGymSearch, buildGroupedDashboardNavigation, buildMobileDashboardNavigation, buildPageHeader } from "./index.js";
describe("dashboard shell", () => {
    it("builds grouped navigation from route permissions", () => {
        ...
        expect(groups.find((group) => group.key === "operations")?.items.map((item) => item.href)).toEqual(["/access-control"]);
    });
    it("builds shell layout with sidebar, top bar, content region, and account menu", () => {
        ...
        expect(shell.topBar.accountMenu.items.find((item) => item.key === "settings")?.disabled).toBe(true);
    ...
    it("builds reusable page header state with actions, breadcrumbs, and tabs", () => {
        ...
        expect(header.tabs.map((tab) => ({ key: tab.key, active: tab.active, disabled: tab.disabled }))).toEqual([
    ...
    it("builds dashboard homepage summary cards from permitted metrics", () => {
        ...
        expect(home.primaryActions.find((action) => action.label === "Add member")?.disabled).toBe(false);
        expect(home.primaryActions.find((action) => action.label === "Create class")?.disabled).toBe(true);
    ...
    it("builds responsive mobile dashboard navigation", () => {
        ...
        expect(mobileNavigation.groups
            .find((group) => group.key === "people")
            ?.items.map((item) => item.href)).toEqual(["/members", "/check-ins"]);
    ...
    it("builds reusable data table sorting and pagination state", () => {
        ...
        expect(dataTable.columns.find((column) => column.key === "checkIns")?.nextDirection).toBe("asc");
    ...
    it("builds reusable toast notification center state", () => {
        ...
        expect(center.toasts.find((toast) => toast.id === "failed")?.action?.href).toBe("/billing/review");
        expect(center.toasts.find((toast) => toast.id === "saved")?.message).toBe("Jamie Rivera was updated.");
...
//# sourceMappingURL=dashboardShell.test.js.map