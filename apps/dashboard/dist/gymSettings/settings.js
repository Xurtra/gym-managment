...
export function buildGymProfileSettingsPage(gym) {
    return {
...
export function buildGymLogoUploadFlow(gym, pendingLogoUrl) {
    return {
...
export function buildBrandColorSettings(gym) {
    return {
...
export function buildBusinessInformationForm(gym) {
...
export function buildTimezoneLocaleSettings(gym) {
    return {
...
export function buildOperatingHoursEditor(operatingHours) {
    const rows = Object.entries(operatingHours).flatMap(([day, ranges]) => (ranges ?? []).map((range) => ({ day, opensAt: range.opensAt, closesAt: range.closesAt })));
    return {
...
export function buildFeatureFlagSettings(activeFlags, allFlags) {
    return {
