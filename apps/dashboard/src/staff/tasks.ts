...
export interface StaffTaskListView {
  ...
  filters: StaffTaskListFilters & { query: string };
...
const PRIORITIES: StaffTaskPriority[] = ["low", "normal", "high", "urgent"];
const STATUSES: StaffTaskStatus[] = ["open", "in_progress", "completed", "cancelled"];
...
export function createStaffTaskAssignmentSubmission(inputModel: {
  ...
  const submission: StaffTaskAssignmentSubmission = {
...
export function createStaffTaskCompletionSubmission(inputModel: {
  ...
  const submission: StaffTaskCompletionSubmission = {
...
function staffAssigneeOptions(staff: StaffMemberView[], selectedUserId: string | undefined) {
  return staff
    .slice()
...
function matchesTaskFilters(
  task: StaffTaskView,
  filters: StaffTaskListView["filters"],
  staffById: Map<string, StaffMemberView>
) {
...
function compareTasks(left: StaffTaskView, right: StaffTaskView) {
  return (
...
function dueState(task: StaffTaskView, today: Date): StaffTaskDueState {
...
function dueLabel(task: StaffTaskView, state: StaffTaskDueState) {
  if (!task.dueAt) {
    return "No due date";
  }
  if (state === "overdue") {
...
function priorityLabel(priority: StaffTaskPriority) {
  return {
    low: "Low",
    normal: "Normal",
    high: "High",
...
function completionBlockedReason(task: StaffTaskView, completedByUserId: string | undefined) {
...
function staffNameValue(staff: StaffMemberView) {
...
function hasActiveFilters(filters: StaffTaskListView["filters"]) {
