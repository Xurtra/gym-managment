...
describe("AuthService", () => {
  ...
  it("logs in, rotates refresh tokens, and rejects reused refresh tokens", async () => {
    ...
    await expect(services.authService.refresh(login.refreshToken)).rejects.toThrow(
  ...
  it("resets a password and invalidates existing refresh tokens", async () => {
    ...
    await expect(
      services.authService.login({ email: ownerInput.email, password: "NewPassword123" })
    ).resolves.toMatchObject({ user: { email: ownerInput.email } });
  });
  ...
  it("verifies email tokens once", async () => {
    ...
    await expect(
      services.authService.verifyEmail({ token: result.emailVerificationToken })
