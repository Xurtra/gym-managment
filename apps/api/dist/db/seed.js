import { AccessDeviceType, BillingInterval, CheckInMethod, MemberStatus, MembershipStatus, RoleName, UserStatus } from "@gym-platform/constants";
...
const services = config.persistenceDriver === "postgres" ? createPostgresServices(config) : createServices(config);
try {
    ...
    const memberMemberships = await Promise.all(members.map((member) => services.memberMembershipService.assignPlan(gymId, member.id, {
