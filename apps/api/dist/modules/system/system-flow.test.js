...
describe("system API flow", () => {
    ...
    it("creates gym locations through the location endpoint and rejects duplicates", async () => {
        ...
        const { response, data } = await api.request(`/gyms/${gymId}/locations`, json({
            name: "  downtown annex  ",
            address: {
    ...
    it("edits gym locations through the location endpoint and rejects duplicate names", async () => {
        ...
        const gymId = owner.gym?.id ?? "";
        const originalLocation = await ok(`/gyms/${gymId}/locations`, json({
            name: "Main Floor",
            address: {
                line1: "100 Fitness Ave",
                line2: "Suite 100",
        ...
        const otherLocation = await ok(`/gyms/${gymId}/locations`, json({
            name: "Uptown Studio",
            address: {
        ...
        const updatedLocation = await ok(`/gyms/${gymId}/locations/${originalLocation.id}`, json({
            name: "Midtown Studio",
            address: {
        ...
        const { response, data } = await api.request(`/gyms/${gymId}/locations/${originalLocation.id}`, json({ name: otherLocation.name }, owner.accessToken, "PATCH"));
        expect(response.status).toBe(409);
        expect(data.error?.code).toBe("location_name_exists");
    });
    it("archives gym locations through the location endpoint and removes them from active access", async () => {
        ...
        const gymId = owner.gym?.id ?? "";
        const activeLocation = await ok(`/gyms/${gymId}/locations`, json({
            name: "Main Floor",
            address: {
                line1: "100 Fitness Ave",
                city: "New York",
        ...
        const archivedLocation = await ok(`/gyms/${gymId}/locations/${activeLocation.id}`, {
            method: "DELETE",
        ...
        const listedLocations = await ok(`/gyms/${gymId}/locations`, {
            headers: authHeaders(owner.accessToken)
        });
        expect(listedLocations.locations).toHaveLength(0);
        const detail = await api.request(`/gyms/${gymId}/locations/${activeLocation.id}`, { headers: authHeaders(owner.accessToken) });
        expect(detail.response.status).toBe(404);
        expect(detail.data.error?.code).toBe("not_found");
        const update = await api.request(`/gyms/${gymId}/locations/${activeLocation.id}`, json({ name: "Reopened Floor" }, owner.accessToken, "PATCH"));
