...
import { generateOpaqueToken, hashToken, signAccessToken } from "../../infrastructure/security/tokens.js";
import { addDays, addHours, addMinutes } from "../../shared/time.js";
...
export class AuthService {
    ...
    clock;
    options;
    constructor(repositories, tenancyService, clock, options) {
    ...
    async register(input) {
        return this.repositories.transaction(async (repositories) => {
            ...
            const gym = input.gymName
                ...
                : undefined;
            const emailVerificationToken = await this.createPurposeToken(user.id, "email_verification", addHours(now, this.options.emailVerificationTokenTtlHours), repositories);
    ...
    async forgotPassword(input) {
        ...
        const token = await this.createPurposeToken(user.id, "password_reset", addMinutes(this.clock.now(), this.options.passwordResetTokenTtlMinutes));
    ...
    async resendVerification(email) {
        ...
        const token = await this.createPurposeToken(user.id, "email_verification", addHours(this.clock.now(), this.options.emailVerificationTokenTtlHours));
    ...
    async verifyTwoFactorSetup(userId, input) {
        const user = await this.getActiveUser(userId);
    ...
    async currentUser(userId, gymId) {
        const user = await this.repositories.users.getUser(userId);
        if (!user) {
            throw unauthorized();
        }
        const memberships = await Promise.all((await this.repositories.gymUsers.listGymMemberships(user.id)).map(async (membership) => {
    ...
    async createSession(user, gymId, repositories = this.repositories) {
    ...
    createAccessToken(user, gymId) {
        ...
        return signAccessToken(payload, this.options.accessTokenSecret, this.options.accessTokenTtlSeconds, this.clock.now());
    }
    async createRefreshToken(userId, gymId, repositories = this.repositories) {
    ...
    async createPurposeToken(userId, purpose, expiresAt, repositories = this.repositories) {
        const token = generateOpaqueToken();
        const now = this.clock.now();
        const id = randomUUID();
    ...
    async consumePurposeToken(token, purpose) {
    ...
    async consumeRecoveryCode(user, recoveryCode) {
...
export function generateTotpCode(secret, now, stepSeconds = 30) {
...
function verifyTotpCode(secret, code, now) {
    if (!secret) {
        return false;
    }
    const provided = new Uint8Array(Buffer.from(code));
    for (const offset of [-1, 0, 1]) {
        const expected = new Uint8Array(Buffer.from(generateTotpCode(secret, new Date(now.getTime() + offset * 30_000))));
