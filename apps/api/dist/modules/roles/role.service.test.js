...
describe("RoleService", () => {
    it("creates default roles and enforces permission checks", async () => {
        ...
        await expect(services.roleService.requirePermission(gymId, owner.user.id, Permission.StaffRoleAssign)).resolves.toBeUndefined();
    });
    it("assigns roles only inside the same gym membership", async () => {
        ...
        const assigned = await services.roleService.assignRole(gymId, staff.user.id, trainerRole.id, owner.user.id);
        ...
        const reassigned = await services.roleService.assignRole(gymId, staff.user.id, frontDeskRole.id, owner.user.id);
        const removed = await services.roleService.removeStaffAccess(gymId, staff.user.id, owner.user.id, { reason: "No longer on payroll" });
        const auditEntries = await services.roleService.listStaffAuditLogs(gymId);
        const removalAudit = auditEntries.find((entry) => entry.action === StaffAuditAction.AccessRemoved);
        ...
        expect(listedStaff.find((entry) => entry.userId === staff.user.id)?.roleName).toBe(RoleName.Trainer);
    ...
    it("creates and edits custom roles with editable permissions", async () => {
        ...
        await expect(services.roleService.createCustomRole(gymId, {
            name: "Front Desk",
        ...
        await expect(services.roleService.updateCustomRole(gymId, managerRole.id, {
            name: "Manager Plus"
    ...
    it("creates staff invites with selected roles and rejects duplicates", async () => {
        ...
        await expect(services.roleService.inviteStaff(gymId, owner.user.id, {
            email: "trainer@example.com",
            roleId: trainerRole.id
    ...
    it("accepts a staff invite and creates gym access", async () => {
        ...
        await expect(services.authService.acceptStaffInvite({
            token: invite.inviteToken,
            firstName: "Demo",
            lastName: "Frontdesk",
            password: "Password123"
