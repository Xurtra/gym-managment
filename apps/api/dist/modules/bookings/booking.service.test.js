import { BillingInterval, BookingStatus, MemberStatus, MembershipStatus } from "@gym-platform/constants";
...
describe("BookingService", () => {
    it("books until capacity, waitlists overflow, and promotes the next waitlisted member on cancel", async () => {
        ...
        await expect(services.bookingService.createBooking(gymId, session.id, { memberId: members[1].id })).rejects.toThrow(/full/i);
    ...
    it("skips an ineligible first waitlisted member and promotes the next eligible member", async () => {
        ...
        expect(cancelled.promotedBooking?.status).toBe(BookingStatus.Booked);
        expect(listed.find((booking) => booking.id === firstWaitlisted.id)?.status).toBe(BookingStatus.Waitlisted);
        expect(listed.find((booking) => booking.id === secondWaitlisted.id)?.status).toBe(BookingStatus.Booked);
    });
    it("rejects duplicate active spots, full waitlists, and ineligible members", async () => {
        ...
        await expect(services.bookingService.createBooking(gymId, session.id, { memberId: members[0].id })).rejects.toThrow(/already/i);
        await expect(services.bookingService.joinWaitlist(gymId, session.id, { memberId: members[2].id })).rejects.toThrow(/waitlist is full/i);
        await expect(services.bookingService.createBooking(gymId, session.id, { memberId: frozen.id })).rejects.toThrow(/eligible/i);
    ...
    it("requires active memberships and enforces plan class access limits", async () => {
        ...
        await expect(services.bookingService.createBooking(gymId, secondSession.id, { memberId: members[0].id })).rejects.toThrow(/limit/i);
        await expect(services.bookingService.createBooking(gymId, session.id, { memberId: noMembership.id })).rejects.toThrow(/active membership/i);
    });
    ...
    it("lets staff manually book with a capacity override", async () => {
        ...
        await services.bookingService.createBooking(gymId, session.id, { memberId: members[0].id });
        const manualBooking = await services.bookingService.createStaffManualBooking(gymId, session.id, staffUserId, {
            memberId: members[1].id,
...
async function createBookingFixture(services, options) {
    ...
    await Promise.all([firstMember, secondMember, thirdMember].map((member) => services.memberMembershipService.assignPlan(gymId, member.id, {
        planId: plan.id,
...
function createMember(services, gymId, firstName, barcode) {
...
//# sourceMappingURL=booking.service.test.js.map