import { BillingInterval, BookingStatus, CheckInMethod, CheckInStatus, MemberStatus, MembershipStatus } from "@gym-platform/constants";
...
describe("CheckInService", () => {
    ...
    it("denies frozen members and members without an active membership", async () => {
        ...
        const expiredMembership = await createMember(services, gymId, "ExpiredPlan", "MEM-EXPIRED-PLAN", MemberStatus.Active);
    ...
    it("rejects check-ins for locations outside the active gym location set", async () => {
        ...
        await expect(services.checkInService.checkIn(gymId, staffUserId, {
...
async function createCheckInFixture(services) {
    ...
    await Promise.all(members.map((member) => services.memberMembershipService.assignPlan(gymId, member.id, {
        planId: plan.id,
        status: MembershipStatus.Active
    })));
    return {
...
function createMember(services, gymId, firstName, barcode, status) {
...
//# sourceMappingURL=checkIn.service.test.js.map