import { AccessDeviceStatus, AccessDeviceType, AccessEventDecision, LocationStatus, MemberStatus, MembershipStatus, PlanStatus } from "@gym-platform/constants";
import { createHash, randomBytes, randomUUID } from "node:crypto";
import { badRequest, notFound } from "../../http/errors.js";
const activeMemberStatuses = new Set([MemberStatus.Active, MemberStatus.Trial]);
...
export class AccessControlService {
    ...
    async registerDevice(gymId, input) {
    ...
    async rotateDeviceApiKey(gymId, deviceId) {
    ...
    async decide(device, member, occurredAt) {
        ...
        const memberships = (await this.repositories.memberMemberships.listMemberMembershipsForMember(member.id)).filter((membership) => membership.gymId === device.gymId);
        const activeMemberships = memberships.filter((membership) => activeMembershipStatuses.has(membership.status) &&
        ...
        const rules = (await this.repositories.accessControl.listAccessRulesForGym(device.gymId)).filter((rule) => rule.locationId === device.locationId &&
            (!rule.startsAt || rule.startsAt <= occurredAt) &&
            (!rule.endsAt || rule.endsAt >= occurredAt));
        const allowed = rules.some((rule) => rule.allowAllActiveMembers ||
    ...
    async resolveMember(gymId, input) {
        ...
        return (await this.repositories.members.listMembersForGym(gymId)).find((candidate) => candidate.barcode === input.barcode && candidate.status !== MemberStatus.Archived);
    }
    async deviceForApiKey(apiKey) {
    ...
    async getScopedDevice(gymId, deviceId) {
    ...
    async markOfflineIfStale(device, now) {
