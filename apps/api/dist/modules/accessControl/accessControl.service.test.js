import { AccessDeviceStatus, AccessEventDecision, BillingInterval, MemberStatus, MembershipStatus } from "@gym-platform/constants";
...
describe("AccessControlService", () => {
    ...
    it("tracks heartbeats, detects offline devices, and rotates API keys", async () => {
        ...
        const rotated = await services.accessControlService.rotateDeviceApiKey(gymId, registered.device.id);
        ...
        await expect(services.accessControlService.heartbeat({ apiKey: registered.apiKey })).rejects.toThrow(/device/i);
        await expect(services.accessControlService.heartbeat({ apiKey: rotated.apiKey })).resolves.toBeTruthy();
    });
});
async function createAccessFixture(services) {
...
//# sourceMappingURL=accessControl.service.test.js.map