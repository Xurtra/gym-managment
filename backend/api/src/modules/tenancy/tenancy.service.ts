...
export class TenancyService {
  ...
  async createGym(ownerUserId: string, input: GymCreateInput) {
  ...
  async resolveGymForUser(userId: string, requestedGymId?: string) {
    const memberships = (await this.repositories.gymUsers.listGymMemberships(userId)).filter(
  ...
  async ensureGymAccess(userId: string, gymId: string) {
  ...
  private async uniqueSlug(baseSlug: string, repositories = this.repositories) {
...
function cleanBrandColors(input: NonNullable<GymUpdateInput["brandColors"]>): NonNullable<Gym["brandColors"]> {
  const colors: NonNullable<Gym["brandColors"]> = { primary: input.primary };
  if (input.secondary) colors.secondary = input.secondary;
  if (input.accent) colors.accent = input.accent;
  return colors;
}

function cleanBusinessInfo(input: NonNullable<GymUpdateInput["businessInfo"]>): NonNullable<Gym["businessInfo"]> {
  const business: NonNullable<Gym["businessInfo"]> = {};
  if (input.legalName) business.legalName = input.legalName;
  if (input.phone) business.phone = input.phone;
  if (input.email) business.email = input.email;
  if (input.website) business.website = input.website;
  if (input.taxId) business.taxId = input.taxId;
  if (input.address) {
    const address = {
