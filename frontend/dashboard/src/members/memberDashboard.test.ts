...
describe("member dashboard screens", () => {
  ...
  it("builds member profile page with details, memberships, and actions", () => {
    ...
    const memberships: MemberProfileMembershipView[] = [
      {
        id: "membership-old",
        gymId: "gym-1",
        memberId: "member-1",
    ...
    expect(
      profile.sections.find((section) => section.key === "emergency_contact")?.details
    ).toContainEqual({
      key: "relationship",
      label: "Relationship",
      value: "Spouse"
    ...
    expect(profile.memberships[0]?.detailHref).toBe(
      "/members/member-1/memberships/membership-active"
    );
