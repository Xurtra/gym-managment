    ...
    <div class="club-home-grid">
      <section class="club-panel club-customers">
        <div class="card-head">
          ...
          <span class="club-kicker">${state.members.length} consumers · ${activeCount} members · ${leadCount} leads</span>
        </div>
        ${spotlight ? `
  ...
  return `
    <section class="club-panel club-page consumer-page">
      <div class="card-head">
        ...
        <span class="club-kicker">${summary.total} total · ${summary.members} members · ${summary.customers} customers · ${summary.leads} leads</span>
      </div>
...
function renderConsumerCreatePanel() {
  ...
  const recurringPlanChoices = [
    ...
    ...recurringPlans.map((plan) => ({ value: plan.id, label: `${plan.name} · ${formatCurrency(plan.priceCents)}` }))
  ];
  return `
...
function renderCustomerProfileView() {
  ...
  return `
    <div style="margin-bottom:1rem;">
      <button class="tab-btn active" data-dashboard-view="consumers" data-preserve-context="true">← Back to Consumers</button>
    </div>
