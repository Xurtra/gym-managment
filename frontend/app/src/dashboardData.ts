...
import type { ClassSessionCreateInput, LocationCreateInput } from "@gym-platform/validation";
import type {
  AccessDeviceStatus,
  AccessDeviceType,
  AccessEventDecision,
...
export async function loadDashboardWorkspaceData(client = createDashboardClient()) {
  ...
  const [consumerResponse, classTypeResponse, locationResponse, staffResponse, scheduleResponse, planResponse] =
    await Promise.all([
      ...
      safeLoad(() => client.listMembershipPlans(gym.id) as Promise<MembershipPlanListResponse | unknown[]>, [])
    ]);
  const plans = Array.isArray(planResponse) ? planResponse : planResponse.plans ?? [];
  ...
  return {
    ...
    plans: normalizePlanSummaries(plans)
  };
}
...
function normalizePlanSummaries(plans: unknown[]): PlanRecord[] {
  return plans.flatMap((record) => {
    const plan = record as Partial<MembershipPlanView>;
...
export async function createLocation(
  gymId: string,
  input: LocationCreateFormInput,
  client = createDashboardClient()
) {
  return client.createLocation(gymId, {
    name: input.name,
    address: {
...
export async function cancelReservation(
  ...
  return client.cancelFacilityReservation(gymId, reservationId, input);
}
