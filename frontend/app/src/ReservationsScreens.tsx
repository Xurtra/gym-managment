...
import { EmptyState, FormLayout, SelectField, Table } from "@gym-platform/ui-react";
import {
  cancelReservation,
  createReservation,
...
function ReservationFormView({
  ...
  const [activeReservation, setActiveReservation] = useState<FacilityReservationRecord | undefined>(reservation);
  ...
  const cancelReservationMutation = useMutation({
    mutationFn: ({ reservationId: id, reason }: { reservationId: string; reason?: string }) => cancelReservation(data.gym.id, id, { reason }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: queryKeys.operationsWorkspace })
  });

  const defaultTimes = defaultReservationTimes();
  const model = buildFacilityReservationCreateScreen({
    ...
    startsAt: reservation?.startsAt ?? defaultTimes.startsAt,
    endsAt: reservation?.endsAt ?? defaultTimes.endsAt,
    note: ""
  });
  ...
  return (
    ...
    >
      ...
      <form
        id="create-resource-reservation-form"
        ...
        onSubmit={(event) => {
          ...
          void (async () => {
            try {
              ...
              const created = await createReservationMutation.mutateAsync({
                resourceId,
                memberId,
              ...
              setSuccess("Resource reservation created.");
            } catch (caught) {
      ...
      >
        ...
        <label className="field">
          <span>{model.fields.note.label}</span>
          <input name="note" defaultValue={model.fields.note.value} disabled={createReservationMutation.isPending} />
        </label>
        <div className="customer-edit-footer">
          <button className="save-button" type="submit" disabled={model.resourceOptions.length === 0 || model.memberOptions.length === 0 || createReservationMutation.isPending}>
      ...
      {activeReservation ? (
        <ReservationDetailCard
          ...
          onCancel={(reason) => {
            ...
            void (async () => {
              try {
                const cancelled = await cancelReservationMutation.mutateAsync({
                  reservationId: activeReservation.id,
                  reason
                }) as FacilityReservationRecord;
                setActiveReservation(cancelled);
                setSuccess("Resource reservation cancelled.");
              } catch (caught) {
                setError(describeError(caught));
              }
            })();
...
function ReservationDetailCard({
  ...
  const resource = data.resources.find((item) => item.id === reservation.resourceId);
  const member = data.members.find((item) => item.id === reservation.memberId);
  const location = data.locations.find((item) => item.id === (reservation.locationId ?? resource?.locationId));
  const canCancel = reservation.status !== "cancelled" && !disabled;

  return (
    <div className="data-card">
      <div className="card-head">
        <div>
          <h3>{resource?.name ?? reservation.resourceId}</h3>
          <p className="club-copy">{formatDateTimeRange(reservation.startsAt, reservation.endsAt)}</p>
        </div>
        <span>{formatStatus(reservation.status)}</span>
      </div>
      <div className="stat-grid compact">
        <article className="mini-card"><span>Customer</span><strong>{member ? `${member.firstName} ${member.lastName}`.trim() : reservation.memberId}</strong></article>
        <article className="mini-card"><span>Location</span><strong>{location?.name ?? reservation.locationId ?? resource?.locationId ?? "Unassigned"}</strong></article>
        <article className="mini-card"><span>Price</span><strong>{formatCurrency(reservation.amountCents)}</strong></article>
        <article className="mini-card"><span>Payment</span><strong>{formatStatus(reservation.paymentStatus)}</strong></article>
      </div>
      {reservation.note ? <div className="club-note"><span className="club-note-label">Note</span><p>{reservation.note}</p></div> : null}
      <form
        id="cancel-resource-reservation-form"
        className="compact-form reservation-cancel-form"
        onSubmit={(event) => {
          event.preventDefault();
          const form = new FormData(event.currentTarget);
          onCancel(optional(form.get("reason")));
        }}
      >
        <input type="hidden" name="reservationId" value={reservation.id} />
        <label className="field">
          <span>Cancellation reason</span>
          <input name="reason" disabled={!canCancel} />
        </label>
        <button className="danger-button" type="submit" disabled={!canCancel}>Cancel reservation</button>
      </form>
    </div>
  );
}
